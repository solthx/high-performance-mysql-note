...
1. 定义数据库、表、及字段的命名规范
    - 因为是共同开发，所以要统一规范
    - 可读性原则、 表意性原则、 长名原则(少用缩写)
2. 选择合适的存储引擎
    - 根据业务选取合适的存储引擎
3. **为表中的字段选择合适的数据类型**
    - tinyint、smallint、mediumint、int、bigint ： 1字节，2字节，3字节，4字节，8字节。
    - float, double: 4字节，8字节，不是精确类型。
    - decimal: 9字节，是精确类型。
    - varchar : 
        - 存储变长字符串，只占用必要的存储空间 （varchar的单位是字符！！不是字节）, 但需要额外一个字节或者两个字节来存储字符串的长度， 即：列的最大长度小于等于255时， 则只用1个额外长度，来记录字符串的长度； 当列的最大长度超过255时，则需要两个字符来记录额外长度；例如(VARCHAR(10) 占用11个字节，VARCHAR(1000)占用1002个字节) , 故最大宽度为65535，但当字符串很长时，不建议使用varchar类型；
        - varchar长度的选择: 使用最小的符合需求的长度
        - varchar的适用场景:
            - 字符串列的最大长度比平均长度大很多
            - **很少被更新的字符串** ， 因为变长的更新，可能会带来内存碎片的问题。
            - 使用了多字节字符集存储字符串（例如中文）
    - char: 
        - 存储定长字符串
        ...
        - char类型的最大宽度为255
        - char类型的适用场景:
            - char类型适合存储长度近似的值 (像身份证之类的。。)
            - char类型适合存储短字符串 ( 例如性别 )
            - char类型**适合存储经常被更新的字符串**
    
    - DATATIME类型 ( 8字节 )
        - 以YYYY-MM-DD HH:MM:SS[.fraction] 格式存储日期时间
        - datatime = YYYY-MM-DD HH:MM:SS 
        - (保存微秒) datatime(6) = YYYY-MM-DD HH:MM:SS.fraction
        - DATATIME类型与时区无关，占用8个字节的存储空间。    
        
    - TIMESTAMP类型: 
        - 时间戳，从1970年1月1日开始到现在的秒数; 
        - 只占用4个字节; 只能保存[ 1970-01-01 ~ 2038-01-19 ] 
        - timestamp的显示依赖所指定的时区. （set time_zone="+10:00" 来改变时区，北京是+8:00）
        - 会自动更新( 即，如果插入不指定的话，会默认是当前时间, 但只会更新一个，即只对 从左往右的第一个timestamp类型进行自动更新 )
    
    - date类型：
        - 占用的字符串比使用字符串、datetime、int存储要少，使用date类型只需要**3个字节**.
        - 使用Date类型还可以利用日期时间函数进行日期之间的计算
        - 时间范围为: [1000-01-01, 9999-12-31]
        - 格式为"YYYY-MM-DD"
...
4. 建立数据库结构
    为Innodb选择主键:
       - 主键尽可能的小 （后面会有介绍
       - 主键应该是顺序增长的，增长数据的插入效率
       - Innodb的主键和业务主键可以不同( 如果业务主键可以满足上面两个特点，那么就可以作为Innodb主键，否则尽量还是选择像自增id这样的来作为Innodb主键 )